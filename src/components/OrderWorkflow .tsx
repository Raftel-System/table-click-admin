...
const OrderWorkflow: React.FC<OrderWorkflowProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ REDIRECTION IMMÉDIATE VERS LA PAGE DES COMMANDES
      navigate('/talya-bercy');
      ...
      if (isCreationError) {
        ...
        // ✅ REDIRECTION IMMÉDIATE MÊME EN CAS D'ERREUR D'IMPRESSION
        navigate('/talya-bercy');
      }
    } finally {
      setLoading(false);
...
{categories.find(cat => cat.id === selectedCategory)?.nom?.toLowerCase().includes('menus sandwichs') && (
  <>
    {/* Debug - supprimez après test */}
    {console.log('✅ Catégorie détectée:', categories.find(cat => cat.id === selectedCategory)?.nom)}
    
    {/* 1. Sandwich seul */}
    <Card className="bg-gray-800/50 border-gray-700 hover:border-yellow-500/50 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl hover:shadow-yellow-500/10">
      <CardContent className="p-6">
        <div className="text-center mb-4">
          <div className="text-5xl mb-3">🥪</div>
          <h3 className="text-xl font-bold text-white mb-2">Sandwich au choix</h3>
          <p className="text-gray-400 text-sm mb-3 line-clamp-2">
            Sandwich au choix (poulet, bœuf, taouk, sawda, falafel).
          </p>
          <div className="flex items-center justify-center gap-2 mb-4">
            <span className="text-2xl font-bold text-yellow-500">7.50{currency}</span>
            <Badge className="bg-blue-500/20 text-blue-400 border border-blue-500/30">
              <ChefHat size={12} className="mr-1" />
              Menu
            </Badge>
          ...
          <Button
            onClick={() => {
              const mockItem: MenuItem = {
                id: 'sandwich-seul-virtual',
                nom: 'Sandwich au choix',
                prix: 7.5,
                description: 'Sandwich au choix (poulet, bœuf, taouk, sawda, falafel)',
                disponible: true,
                categorieId: selectedCategory,
                ordre: 999
              };
              handleComposedMenuClick(mockItem);
            }}
            className="w-full h-12 bg-blue-500/20 border-2 border-blue-500/50 hover:bg-blue-500 text-blue-400 hover:text-white rounded-xl transition-all duration-300"
          >
            <ChefHat size={20} className="mr-2" />
    ...
    <Card className="bg-gray-800/50 border-gray-700 hover:border-yellow-500/50 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl hover:shadow-yellow-500/10">
      <CardContent className="p-6">
        <div className="text-center mb-4">
          <div className="text-5xl mb-3">🥪🥤</div>
          <h3 className="text-xl font-bold text-white mb-2">Sandwich + Boisson</h3>
          <p className="text-gray-400 text-sm mb-3 line-clamp-2">
            Sandwich au choix + Boisson (33cl).
          </p>
          <div className="flex items-center justify-center gap-2 mb-4">
            <span className="text-2xl font-bold text-yellow-500">9.50{currency}</span>
            <Badge className="bg-blue-500/20 text-blue-400 border border-blue-500/30">
              <ChefHat size={12} className="mr-1" />
              Menu
            </Badge>
        ...
        <div className="flex items-center justify-center gap-4">
          <Button
            onClick={() => {
              const mockItem: MenuItem = {
                id: 'sandwich-boisson-virtual',
                nom: 'Sandwich + Boisson',
                prix: 9.5,
